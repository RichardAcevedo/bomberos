<template>
  <div class="q-py-lg q-px-xl bg-grey-2 background-height">
    <div class="row q-mb-xs">
      <div class="title">CARGOS</div>
      <InteractionOptions
        v-if="user.type !== 'supervisor' && positions && positions.length"
        :selected="selected"
    ...
    <TableView
      v-else-if="positions && positions.length"
      :key="reRenderTable"
    ...
    <q-dialog v-model="alterPositions">
      <q-card style="width: 480px; max-width: 80vw">
        ...
        />
        <q-card-section>
          <div class="text-h6 text-bold">Cargo</div>
        </q-card-section>
        <q-card-section>
          <q-form
            @submit="alterRow"
...
import { titles, staticColumns } from '../data/titlesBanks/positions';
...
export default {
  ...
  methods: {
    ...
    async openUpdate() {
      ...
      const position = await this.$store.dispatch('positions/getForId', this.selected[0].id);
    ...
    async alterRow() {
      ...
      if (this.isCreate) {
        try {
          const dataFormat = await positionModel.getApiFormat(this.positionData);
          await this.$store.dispatch('positions/store', dataFormat);
          const msg = 'Registro exitoso';
          this.getData();
          this.alterPositions = false;
        ...
        try {
          const dataFormat = await positionModel.getApiFormat(this.positionData);
          await this.$store.dispatch('positions/update', dataFormat);
    ...
    async deleteRows(infoDelete) {
      await this.$store.dispatch('positions/remove', infoDelete);
    },
    reset() {
      this.positionData = {
        name: null,
    ...
    async getData() {
      await this.$store.dispatch('positions/get');
  ...
  computed: {
    ...
    positions() {
      return this.$store.getters['positions/list'];
    },
    columns() {
      return getColumnsForTable(this.positions, titles, staticColumns);
    },
  },
