<template>
  <div class="q-py-lg q-px-xl bg-grey-2 background-height">
    <div class="row q-mb-xs">
      ...
      <InteractionOptions
        v-if="user.type !== 'supervisor' && bills && bills.length"
        :selected="selected"
...
export default {
  ...
  methods: {
    createNew() {
      this.$q.localStorage.set(LS_CREATING_BILLS, { created: true });
      this.$router.replace(ROUTES.bills.alterBills);
    },
    editRow() {
      this.$q.localStorage.set(LS_CREATING_BILLS, { created: false, id: this.selected[0].id });
      this.$router.replace(ROUTES.bills.alterBills);
    },
    organizerDataDelete(info) {
      return info.reduce((m, item) => {
        m.push({
          principal: item.issueType,
          caption: item.date,
          side: item.userSession,
        });
        return m;
      }, []);
    },
