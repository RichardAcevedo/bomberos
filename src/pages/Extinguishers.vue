<template>
  <div class="q-py-lg q-px-xl bg-grey-2 background-height">
    <div class="row q-mb-xs">
      <div class="title">EXTINTORES</div>
      <InteractionOptions
        v-if="user.type !== 'supervisor' && extinguishers && extinguishers.length"
        :selected="selected"
    ...
    <TableView
      v-else-if="extinguishers && extinguishers.length"
      :key="reRenderTable"
      :information="extinguishers"
      :columns="columns"
      :visibleColumns="visibleColumns"
      @selection="selected = $event"
    />
    <div v-else class="text-center">
      <div class="empty-state normal-section empty-extinguishers"></div>
...
import { titles, staticColumns } from '../data/titlesBanks/extinguishers';
...
export default {
  ...
  methods: {
    createNew() {
      this.$q.localStorage.set(LS_CREATING_EXTINGUISHERS, { created: true });
      this.$router.replace(ROUTES.extinguishers.alterExtinguishers);
    },
    editRow() {
      this.$q.localStorage.set(LS_CREATING_EXTINGUISHERS,
    ...
    async deleteRows(infoDelete) {
      await this.$store.dispatch('extinguishers/remove', infoDelete);
    },
    getData() {
      this.$store.dispatch('extinguishers/get');
    },
  },
  computed: {
    user() {
    ...
    extinguishers() {
      return this.$store.getters['extinguishers/list'];
    },
    columns() {
      return getColumnsForTable(this.extinguishers, titles, staticColumns);
    },
  },
