<template>
  <div class="q-py-lg q-px-xl bg-grey-2 background-height">
    <div class="row q-mb-xs">
      <div class="title">ASCENSOS</div>
      <InteractionOptions
        v-if="user.type !== 'supervisor' && promotions && promotions.length"
        :selected="selected"
    ...
    <TableView
      v-else-if="promotions && promotions.length"
      :key="reRenderTable"
    ...
    <div v-else class="text-center">
      <div class="empty-state normal-section empty-promotions"></div>
...
import { titles, staticColumns } from '../data/titlesBanks/promotions';
...
export default {
  ...
  methods: {
    createNew() {
      this.$q.localStorage.set(LS_CREATING_PROMOTIONS, { created: true });
      this.$router.replace(ROUTES.personal.alterPromotions);
    },
    editRow() {
      this.$q.localStorage.set(LS_CREATING_PROMOTIONS, { created: false, id: this.selected[0].id });
      this.$router.replace(ROUTES.personal.alterPromotions);
    },
    organizerDataDelete(info) {
      return info.reduce((m, item) => {
        m.push({
          ...
          caption: item.active === 'Si' ? 'Activo' : 'Inactivo',
          side: item.employee,
        });
        return m;
      }, []);
    ...
    async deleteRows(infoDelete) {
      await this.$store.dispatch('promotions/remove', infoDelete);
    },
    getData() {
  ...
  computed: {
    ...
    promotions() {
      return this.$store.getters['promotions/list'];
    },
    columns() {
      return getColumnsForTable(this.promotions, titles, staticColumns);
    },
  },
