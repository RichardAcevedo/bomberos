<template>
  <div class="q-py-lg q-px-xl bg-grey-2 background-height">
    <div class="row q-mb-xs">
      ...
      <InteractionOptions
        v-if="user.type !== 'supervisor' && radios && radios.length"
        :selected="selected"
    ...
    <TableView
      v-else-if="radios && radios.length"
      :key="reRenderTable"
      :information="radios"
      :columns="columns"
    ...
    <div v-else class="text-center">
      <div class="empty-state normal-section empty-radios"></div>
...
import { titles, staticColumns } from '../data/titlesBanks/radios';
...
export default {
  ...
  methods: {
    createNew() {
      this.$q.localStorage.set(LS_CREATING_RADIOS, { created: true });
      this.$router.replace(ROUTES.vehicles.alterRadios);
    },
    editRow() {
      this.$q.localStorage.set(LS_CREATING_RADIOS, { created: false, id: this.selected[0].id });
      this.$router.replace(ROUTES.vehicles.alterRadios);
    },
    organizerDataDelete(info) {
      return info.reduce((m, item) => {
        m.push({
          principal: item.serial,
          caption: item.employee,
          side: item.state,
        });
        return m;
      }, []);
    ...
    async deleteRows(infoDelete) {
      await this.$store.dispatch('radios/remove', infoDelete);
    },
    getData() {
      this.$store.dispatch('radios/get');
    },
  },
  computed: {
    user() {
    ...
    radios() {
      return this.$store.getters['radios/list'];
    },
    columns() {
      return getColumnsForTable(this.radios, titles, staticColumns);
    },
  },
